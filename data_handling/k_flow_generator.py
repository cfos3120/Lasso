...
if __name__ == '__main__':
    ...
    # Initialize Solution Storing Vector and start time
    sol = np.zeros((T, t + 1, s // sub, s // sub, 3))
    t1 = default_timer()

    # Store initial solution for Vorticity and Cartesian Velocity Components
    sol_ini_w = NS.vorticity().squeeze(0).cpu().numpy()[::sub, ::sub]
    ...
    # Loop over T, essentially, same flow but split into batches ever t timesteps for T batches
    for i in range(T):
        
        sol[i, 0, :, :, 2] = NS.vorticity().squeeze(0).cpu().numpy()[::sub, ::sub]
        sol[i, 0, :, :, 0], sol[i, 0, :, :, 1] = sol_ini_x, sol_ini_y
        for j in range(t):
            t2 = default_timer()
            print(f'Preparing Batch: {int(i)} Timestep: {int(j)}. Time Taken: {(t2 - t1):.3f}s', end='\r')
            # Advance Timestep
            NS.advance(dt, delta_t=1e-3)

            # Store Vorticity Solution
            sol[i, j + 1, :, :, 2] = NS.vorticity().squeeze(0).cpu().numpy()[::sub, ::sub]

            # Store Cartesian Velocities
    ...
    file_name = f'NS_fine_Re{int(Re)}_T{t}_R{s}_sub{sub}_3channel.npy'
    print(f'Saving results as {file_name}', end='\n')
    np.save(file_name, sol)


## THE UP TO DATE REPO HAS A KOLOMOGOROV FUNCTION WHERE THE FORCING FIELD CAN BE SPECIFIED
## THIS MEANS WE CAN CREATE OUR OWN FORCING IN X-Y AND CALCULATE THE VORITCITY EQUIVALENT USING THE DIVERGENCE
## CREATE OUR OWN DATASET AND ENFORCE IT WITH CARTESIAN PDES IN FNO MODEL
## ADDITIONALLY, LOOK AT PUTTING IN OBJECTS INTO SPECTRAL SOLVER